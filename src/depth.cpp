//
// Created by JeremyGuo on 2022/2/24.
//
...
void MyApp::initGraphicsPipeline() {
    auto vertShaderCode = readFile("depth.vert.spv");
    auto fragShaderCode = readFile("depth.frag.spv");
...
void MyApp::initSyncObjects() {
    ...
    for (auto &frame : frameInfos)
        if (vkCreateSemaphore(device, &semaphoreInfo, nullptr, &frame.imageAvailableSemaphore) != VK_SUCCESS ||
            ...
            std::throw_with_nested(std::runtime_error("failed to create semaphores!"));
        }
}
...
void MyApp::createBuffer(VkDeviceSize size, VkBufferUsageFlags usage, VkMemoryPropertyFlags properties,
...
uint32_t MyApp::findMemoryType(uint32_t typeFilter, VkMemoryPropertyFlags properties) {
...
void MyApp::copyBuffer(VkBuffer srcBuffer, VkBuffer dstBuffer, VkDeviceSize size) {
    try {
        ...
        std::throw_with_nested(std::runtime_error("failed to copy buffer"));
    }
}
...
void MyApp::createImage(uint32_t width, uint32_t height, VkFormat format, VkImageTiling tiling, VkImageUsageFlags usage,
...
VkCommandBuffer MyApp::beginSingleTimeCommands() {
    VkCommandBufferAllocateInfo allocInfo{};
    allocInfo.sType = VK_STRUCTURE_TYPE_COMMAND_BUFFER_ALLOCATE_INFO;
    allocInfo.level = VK_COMMAND_BUFFER_LEVEL_PRIMARY;
    allocInfo.commandPool = commandPool;
...
void MyApp::endSingleTimeCommands(VkCommandBuffer commandBuffer) {
...
void MyApp::transitionImageLayout(VkImage image, VkFormat format, VkImageLayout oldLayout, VkImageLayout newLayout) {
    VkCommandBuffer commandBuffer = beginSingleTimeCommands();
...
void MyApp::copyBufferToImage(VkBuffer buffer, VkImage image, uint32_t width, uint32_t height) {
    VkCommandBuffer commandBuffer = beginSingleTimeCommands();
...
void MyApp::initTextureView() {
    try {
        textureImageView = createImageView(textureImage, VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_ASPECT_COLOR_BIT);
    } catch(...) {
        std::throw_with_nested(std::runtime_error("failed to create texture view"));
    }
}

VkImageView MyApp::createImageView(VkImage image, VkFormat format, VkImageAspectFlags aspectFlags) {
...
void MyApp::initTextureSampler() {
    try {
        ...
        std::throw_with_nested(std::runtime_error("failed to create texture sampler"));
    }
}
